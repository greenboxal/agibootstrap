...
ValuePtr NewV8Array(ContextPtr ctx, uint32_t length) {
  LOCAL_CONTEXT(ctx)

  v8::Local<v8::Array> array = v8::Array::New(iso, length);

  m_value* val = new m_value;
  val->id = 0;
  val->iso = iso;
  val->ctx = ctx;
  val->ptr = Persistent<Value, CopyablePersistentTraits<Value>>(iso, array);
  return tracked_value(ctx, val);
}
