...
func (f *methodProperty) AsObservable(receiver Value) obsfx2.Observable {
	return nil
}
...
func (f *funcProperty) AsObservable(receiver Value) obsfx2.Observable {
	return nil
}
...
func (r *fieldProperty) AsObservable(receiver Value) obsfx2.Observable {
	if r.IsObservable() {
		v := r.GetRawValue(receiver)
		obs := v.Interface().(obsfx2.Observable)

		return obs
	} else {
		return obsfx2.BindExpression[any](func() any {
			v := r.GetRawValue(receiver)
