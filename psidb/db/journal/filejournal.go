...
func (j *FileJournal) Iterate(startIndex uint64, count int) iterators.Iterator[coreapi.JournalEntry] {
	index := startIndex
	total := 0
...
func (j *FileJournal) Read(index uint64, dst *coreapi.JournalEntry) (*coreapi.JournalEntry, error) {
...
func (j *FileJournal) Write(op *coreapi.JournalEntry) error {
	j.mu.Lock()
	defer j.mu.Unlock()
...
func (j *FileJournal) GetHead() (uint64, error) {
